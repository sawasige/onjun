...
sub regUser() {
	...
	if (&isMobile()) {
		# セッションは URL 埋め込み
		print $cgi->redirect("reguserconfirm.cgi?$config{'sessionname'}=$sid");
	} else {
		# セッションは Cookie 埋め込み
		print $cgi->redirect(
			-uri=>"reguserconfirm.cgi",
			-cookie=>$cgi->cookie(-name=>$config{'sessionname'}, -value=>$session->id));
	}
	return 1;
}
