...
sub main {
	...
	if (defined $session) {
		...
		if ($cgi->param('cancel')) {
			# 画面リダイレクト
			my $url = 'receivemessageview.cgi?messageid='.$cgi->param('messageid');
			if (&isMobile()) {
			...
			if (!$check) {
				...
				# 現在の画面
				$msg .= &checkOnline($dbh, $session->param('userid'), '受信メッセージ削除');
...
sub disp {
	...
	if ($message{'subject'}) {
		# 送信者
		if ($tmpl->query(name => 'SENDER_NAME') eq 'VAR') {
			$tmpl->param(SENDER_NAME => &convertOutput($message{'sender_name'}));
		}
		# 送信時間
...
sub getMessage() {
	...
	if ($messageid) {
		...
		my ($subject, $body, $sendtime, $sender_name) = &selectFetchArray($dbh, $sql, @bind);
		if ($subject) {
...
sub deleteMessage() {
	...
	my $sql = 'UPDATE messages SET receiver_deleteflag=? WHERE messageid=? AND receiver_userid=?';
	&doDB($dbh, $sql, @bind);
	...
	if (&isMobile()) {
		# セッションは URL 埋め込み
		print $cgi->redirect("receivemessagelist.cgi?$config{'sessionname'}=$sid");
	} else {
		# セッションは Cookie 埋め込まれている
		print $cgi->redirect('receivemessagelist.cgi');
	}
