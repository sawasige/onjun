...
sub main {
	...
	if (defined $session) {
		...
		# 現在の画面
		$msg .= &checkOnline($dbh, $session->param('userid'), '受信メッセージ一覧');
...
sub disp {
	...
	if (@messages && $tmpl->query(name => 'MESSAGES') eq 'LOOP') {
		...
		foreach my $row(@messages) {
			my ($messageid, $subject, $senderid, $sendername, $time, $opentime) = @$row;
			$opentime =~ s/[^\d]//g;
			$opentime += 0;
			my %message;
			if ($tmpl->query(name => ['MESSAGES', 'URL']) eq 'VAR') {
				my $url = 'receivemessageview.cgi?messageid='.$messageid;
				if (&isMobile) {
			...
			if ($tmpl->query(name => ['MESSAGES', 'SENDER']) eq 'VAR') {
				$message{'SENDER'} = &convertOutput($sendername);
			}
			if ($tmpl->query(name => ['MESSAGES', 'SENDERURL']) eq 'VAR') {
				my $url = 'profile.cgi?userid='.$senderid;
				if (&isMobile) {
					$url .= '&'.$config{'sessionname'}.'='.$session->id;
				}
				$message{'SENDERURL'} = &convertOutput($url);
			}
			if ($tmpl->query(name => ['MESSAGES', 'TIME']) eq 'VAR') {
				$message{'TIME'} = &convertOutput($time);
			}
			if ($opentime && $tmpl->query(name => ['MESSAGES', 'OPENED']) eq 'VAR') {
				$message{'OPENED'} = 1;
			}
...
sub getList() {
	...
	my $sql = 'SELECT a.messageid, a.subject, a.sender_userid, b.name, a.sendtime, a.opentime  FROM messages a, users b';
	my $sqlwhere = ' WHERE a.sender_userid=b.userid AND a.receiver_userid=? AND a.receiver_deleteflag=? ';
	$sqlcount .= $sqlwhere;
